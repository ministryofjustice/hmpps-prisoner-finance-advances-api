...
build:
	docker build -t hmpps-prisoner-finance-advances-api .
...
serve-environment:
	docker compose up --scale hmpps-prisoner-finance-advances-api=0 -d --wait

serve-db:
	docker compose up --scale hmpps-prisoner-finance-advances-api=0 --scale hmpps-auth=0 -d --wait
